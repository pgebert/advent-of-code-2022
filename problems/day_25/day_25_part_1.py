...
class Floor:
    ...
    def __init__(self, input: List[str]):
        self.state = [[character for character in line] for line in input]
    ...
    def step(self):
        ...
        # east
        new_state = [["." for _ in line] for line in self.state]
        for row in range(len(self.state)):
            for col in range(len(self.state[row])):

                if self.state[row][col] == ">" and self.state[row][(col + 1) % len(self.state[row])] == ".":
                    new_state[row][(col + 1) % len(new_state[row])] = self.state[row][col]
        ...
        for row in range(len(self.state)):
            for col in range(len(self.state[row])):
                ...
                if self.state[row][col] == "v" and self.state[(row + 1) % len(self.state)][col] == ".":
                    new_state[(row + 1) % len(self.state)][col] = self.state[row][col]
                elif self.state[row][col] != ".":
                    new_state[row][col] = self.state[row][col]

        self.state = new_state

        return not all((i == j for i, j in zip(self.state, before)))
